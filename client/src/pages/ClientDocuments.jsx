...
export default function Client() {
  ...
  useEffect(() => {
    ...
    const getTotalRemainingCancelledCredits = async () => {
      try {
        const response = await fetch(
          "http://localhost:8080/remaining-credits",
          {
            headers: {
        ...
        setTotalRefundAmount(
          data.reduce((sum, credit) => sum + credit.refundAmount, 0)
        );
    ...
    <div className="min-h-screen bg-black text-white p-8 z-0">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-lg font-light">
          Screen {screenId} at{" "}
          {new Date(showtime).toLocaleString("default", { month: "short" }) +
            " " +
            new Date(showtime).getDate() +
            ", " +
            new Date(showtime).getFullYear() +
            " - " +
      ...
      <div className="grid lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
        ...
        {localStorage.getItem("token") ? (
          ...
          <>
            You need to log in to make a booking. Please log in or sign in to
            make a booking.
          </>
        )}
      </div>
